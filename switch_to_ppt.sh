...
if [ -n "$current_space" ]; then
    if [ "$current_space" -ne 5 ]; then
        echo "Debug: Not on space 5, saving current space and switching to space 5 (WeChat/Chat apps)."
        echo $current_space > /tmp/last_space
        # 切换到桌面 5
        osascript -e "tell application \"System Events\" to key code 23 using {control down}" > /dev/null
        echo "Debug: Switched to space 5 (WeChat/Chat apps)."
    elif [ "$current_space" -eq 5 ]; then
