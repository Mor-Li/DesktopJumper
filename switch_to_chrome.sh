...
# 根据显示器数量定义目标桌面编号 - 浏览器设置在第7个桌面
if [ "$num_displays" -eq 1 ]; then
    target_desktop=7  # 针对单显示器的设定，浏览器在第7个桌面
elif [ "$num_displays" -eq 2 ]; then
    target_desktop=7  # 针对双显示器的设定，浏览器在第7个桌面
elif [ "$num_displays" -eq 3 ]; then
    target_desktop=7  # 针对三显示器的设定，浏览器在第7个桌面
else
    target_desktop=7  # 针对更多显示器的设定，浏览器在第7个桌面
fi
