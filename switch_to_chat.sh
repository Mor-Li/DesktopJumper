...
if [ -n "$current_space" ]; then
    if [ "$current_space" -ne 1 ]; then
        echo "Debug: Not on space 1, saving current space and switching to space 1 (WeChat/Chat apps)."
        echo $current_space > /tmp/last_space
        # 切换到桌面 1
        osascript -e "tell application \"System Events\" to key code 18 using {control down}" > /dev/null
        echo "Debug: Switched to space 1 (WeChat/Chat apps)."
    elif [ "$current_space" -eq 1 ]; then
