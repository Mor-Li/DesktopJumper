...
# 根据显示器数量定义目标桌面编号 - 主桌面设置在第1个桌面
if [ "$num_displays" -eq 1 ]; then
    target_desktop=1  # 针对单显示器的设定，主桌面在第1个桌面
elif [ "$num_displays" -eq 2 ]; then
    target_desktop=1  # 针对双显示器的设定，主桌面在第1个桌面
elif [ "$num_displays" -eq 3 ]; then
    target_desktop=1  # 针对三显示器的设定，主桌面在第1个桌面
else
    target_desktop=1  # 针对更多显示器的设定，主桌面在第1个桌面
fi
