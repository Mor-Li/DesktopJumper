...
if [ -n "$current_space" ]; then
    if [ "$current_space" -ne 8 ]; then
        echo "Debug: Not on space 8, saving current space and switching to space 8 (Music)."
        echo $current_space > /tmp/last_space
        # 切换到桌面 8
        osascript -e "tell application \"System Events\" to key code 28 using {control down}" > /dev/null
        echo "Debug: Switched to space 8 (Music)."
    elif [ "$current_space" -eq 8 ]; then
