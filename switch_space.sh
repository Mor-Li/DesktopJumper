...
# 根据显示器数量定义目标桌面编号 - ChatGPT设置在第三个桌面
if [ "$num_displays" -eq 1 ]; then
    target_desktop=3  # 针对单显示器的设定，ChatGPT在第三个桌面
elif [ "$num_displays" -eq 2 ]; then
    target_desktop=3  # 针对双显示器的设定，ChatGPT在第三个桌面
elif [ "$num_displays" -eq 3 ]; then
    target_desktop=3  # 针对三显示器的设定，ChatGPT在第三个桌面
else
    target_desktop=3  # 针对更多显示器的设定，ChatGPT在第三个桌面
fi
...
# alt - space : zsh "/Users/moonshot/Documents/DesktopJumper/switch_space.sh"
